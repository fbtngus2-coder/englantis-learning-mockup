...
export function buildTeachResult(session: TeachSessionState): TeachResult {
  ...
  return {
    missionTitle: "NPC에게 Let's + base verb 가르치기",
    score: teachScore,
    understanding: session.npcUnderstanding,
    confidence: session.npcConfidence,
    studentSummary:
      "오늘 너는 Milo에게 Let's + 동사원형 규칙을 가르쳤어. 틀린 문장을 고치고, 이유를 설명하고, 새 예문까지 만들어 보였어.",
    parentSummary:
      "오늘 학생은 Let's + 동사원형 표현을 학습한 뒤, NPC의 틀린 문장을 직접 교정하고 왜 틀렸는지 설명했습니다. 단순히 문제를 맞히는 수준을 넘어 배운 내용을 다른 대상에게 설명하는 단계까지 도달했습니다.",
    teacherSummary:
    ...
    nextPractice:
      session.hintUsed > 2
        ? "다음에는 PIX 힌트를 한 단계 늦게 열고, 이유 설명을 한 문장 더 말해보면 좋아요."
        : "다음에는 같은 규칙으로 Let's make, Let's read, Let's go home 문장을 더 만들어보면 좋아요.",
    metrics: [
